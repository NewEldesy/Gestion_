...
function addProduit($data) {
    ...
    // Insérer une entrée dans la table Stock avec la quantité initialisée à 0
    $stockQuery = "INSERT INTO Stock (id_produit, quantite) VALUES (:id_produit, :quantite)";
...
function getTotalFromTables($startDate, $endDate) {
    $database = dbConnect();
    $queries = [
        "SELECT SUM(montant) AS total FROM Mecaniques WHERE date_entree BETWEEN :startDate AND :endDate",
